...
namespace xpl {
  ...
  class postfix_writer: public basic_ast_visitor {
    ...
    std::map<std::string,int> _externs {{"readi", 1}, {"printi", 1}, {"prints", 1}, {"println", 1}, {"argc", 1}, {"argv", 1}, {"envp", 1}};

  public:
    postfix_writer(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<xpl::symbol> &symtab,
                   cdk::basic_postfix_emitter &pf) :
        basic_ast_visitor(compiler), _symtab(symtab), _pf(pf), _lbl(0), _returnlbl(0), _context(false), _offset(0) {
    }
