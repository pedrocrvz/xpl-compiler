...
namespace xpl {
  ...
  protected:
    ...
    void process_expression(cdk::basic_node * const node, int lvl) {
      os() << std::string(lvl, ' ') << "<" << node->label() << "/>" << std::endl;
    }

    void process_bool(bool value, const std::string &name, int lvl) {
      if (value)
        os() << std::string(lvl, ' ') << "<" << name << "/>" << std::endl;
    }

    void process_id(const std::string &id, int lvl) {
      os() << std::string(lvl, ' ') << "<id>" << id << "</id>" << std::endl;
    }

    void process_type(basic_type *type, int lvl) {
      os() << std::string(lvl, ' ') << "<type>" << getType(type) << "</type>" << std::endl;
    }

    const std::string getType(basic_type *type) const {
      switch(type->name()) {
